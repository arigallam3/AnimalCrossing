...
export class NorthBug extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
    ...
    componentDidMount() {
        return this.animalcrossingapi.GetAllAnimalCrossingNorthBugs()
            .then(data => {

                this.setState({ northbugs: data, loading: false });
            });
    }
        ...
        return (
            <React.Fragment>
                <Table className="tablestyle" bordered striped>
                    ...
                    <tbody>
                        {this.state.northbugs.map(record =>
                            <tr key={record.id}>
                                <td>{record.name}</td>
                                <td><img src={record.image} alt="northbug" /></td>
                                <td>{record.price}</td>
