...
export class NorthFish extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
    ...
    componentDidMount() {
        return this.animalcrossingapi.GetAllAnimalCrossingNorthFishes()
            .then(data => {

                this.setState({ northfishes: data, loading: false });
            });
    }
        ...
        return (
            <React.Fragment>
                <Table className="tablestyle" bordered striped >
                        ...
                        <tbody>
                            {this.state.northfishes.map(record =>
                                <tr key={record.id}>
                                    <td>{record.name}</td>
                                    <td><img src={record.image} alt="northfish" /></td>
                                    <td>{record.price}</td>
