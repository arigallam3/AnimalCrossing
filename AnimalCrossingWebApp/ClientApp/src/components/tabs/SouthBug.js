...
export class SouthBug extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
    ...
    componentDidMount() {
        return this.animalcrossingapi.GetAllAnimalCrossingSouthBugs()
            .then(data => {

                this.setState({ southbugs: data, loading: false });
            });
    }
        ...
        return (
            <React.Fragment>
                <Table className="tablestyle" bordered striped>
                    ...
                    <tbody>
                        {this.state.southbugs.map(record =>
                            <tr key={record.id}>
                                <td>{record.name}</td>
                                <td><img src={record.image} alt="southbug" /></td>
                                <td>{record.price}</td>
